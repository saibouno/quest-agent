...
- Let `node scripts/theme-harness.mjs scaffold-closeout` auto-promote durable context when there is a recorded delta.
...
- `theme-ops.mjs explain` owns the structured durable delta, while `scaffold-closeout` owns the repo-local auto-promotion gate.
- `scripts/promote-durable-context.mjs` is the repo-local helper behind that gate and the troubleshooting entrypoint when promotion needs inspection.
